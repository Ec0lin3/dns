...
param(
    ...
    [switch]$SkipAcl,
    [switch]$NoOpen,
    [int]$MaxGroups = 1500,   # safety stop: above this, almost certainly an over-broad name pattern
    [switch]$Force            # bypass the MaxGroups guard
)
        ...
        $defaultConfig = @'
{
  ...
  "discovery": {
    "byName": true,
    "byAdminCount": true,
    "byNestedInSeeds": true,
    "seedPrivilegedGroups": [
  ...
  "groupNamePatterns": [
    ...
    "Tier0*", "T0-*", "*-DA", "*-Tier0", "*PrivAccess*", "*PAW*"
  ],
  "_groupNamePatterns_note": "Name patterns are just ONE discovery source. adminCount=1 and nesting-in-seeds catch org-specific names automatically. Add broad wildcards like '*admin*' only if you use an admin naming convention.",
  "sensitiveRights": [
...
# ----------------------------------------------------------------------------
# 1. Discover sensitive groups — by privilege, not just by name
# ----------------------------------------------------------------------------
# Discovery config (defensive defaults if section absent)
$disc        = if ($cfg.PSObject.Properties.Name -contains 'discovery') { $cfg.discovery } else { $null }
$byName      = if ($disc -and $null -ne $disc.byName)            { [bool]$disc.byName }            else { $true }
$byAdminCnt  = if ($disc -and $null -ne $disc.byAdminCount)      { [bool]$disc.byAdminCount }      else { $true }
$byNested    = if ($disc -and $null -ne $disc.byNestedInSeeds)   { [bool]$disc.byNestedInSeeds }   else { $true }
$seedNames   = if ($disc -and $disc.seedPrivilegedGroups)        { @($disc.seedPrivilegedGroups) } else {
...
Write-Step "Discovering sensitive groups (name + adminCount + nesting)..."
$allGroups = Get-ADGroup -Filter * -SearchBase $searchBase -Properties member, memberOf, sAMAccountName, objectSid, adminCount @adParams
$groupByDN = @{}
foreach ($g in $allGroups) { $groupByDN[$g.DistinguishedName] = $g }

# Accumulator: DN -> group object, tagged with the reason(s) it is sensitive
$found = @{}
function Add-Sensitive {
    param($g, [string]$reason)
    if (-not $g) { return }
    if (-not $found.ContainsKey($g.DistinguishedName)) {
        $g | Add-Member -NotePropertyName Reasons -NotePropertyValue (New-Object System.Collections.Generic.List[string]) -Force
        $found[$g.DistinguishedName] = $g
    }
    if ($found[$g.DistinguishedName].Reasons -notcontains $reason) { $found[$g.DistinguishedName].Reasons.Add($reason) }
}

# 1a. By name pattern
if ($byName) {
    foreach ($g in $allGroups) {
        foreach ($p in $cfg.groupNamePatterns) {
            if ($g.Name -like $p -or $g.sAMAccountName -like $p) { Add-Sensitive $g "name:$p"; break }
        }
    }
}

# 1b. By adminCount=1 (AdminSDHolder/SDProp-protected — name-independent, strongest signal)
if ($byAdminCnt) {
    foreach ($g in $allGroups) { if ($g.adminCount -eq 1) { Add-Sensitive $g 'adminCount=1' } }
}

# 1c. By nesting inside seed privileged groups (recursive) — catches org-specific names
if ($byNested) {
    foreach ($seed in $seedNames) {
        $seedGrp = $allGroups | Where-Object { $_.Name -eq $seed -or $_.sAMAccountName -eq $seed } | Select-Object -First 1
        if (-not $seedGrp) { continue }
        $nested = Get-ADGroup -LDAPFilter "(memberOf:1.2.840.113556.1.4.1941:=$($seedGrp.DistinguishedName))" @adParams
        foreach ($n in $nested) {
            $full = if ($groupByDN.ContainsKey($n.DistinguishedName)) { $groupByDN[$n.DistinguishedName] }
                    else { Get-ADGroup -Identity $n.DistinguishedName -Properties member, memberOf, sAMAccountName, objectSid, adminCount @adParams }
            Add-Sensitive $full "nested-in:$seed"
        }
    }
}

# Materialize, and set MatchedPattern = combined reasons (used by the rest of the script/reports)
$sensitiveGroups = @($found.Values)
foreach ($g in $sensitiveGroups) { $g | Add-Member -NotePropertyName MatchedPattern -NotePropertyValue ($g.Reasons -join '; ') -Force }
Write-Ok "Found $($sensitiveGroups.Count) sensitive group(s)  [name=$byName adminCount=$byAdminCnt nested=$byNested]."

# Breakdown by discovery reason — exposes an over-broad source (e.g. a wildcard matching thousands)
$reasonCounts = @{}
foreach ($g in $sensitiveGroups) { foreach ($r in $g.Reasons) { $reasonCounts[$r] = 1 + ($(if ($reasonCounts.ContainsKey($r)) { $reasonCounts[$r] } else { 0 })) } }
Write-Host "    Discovery breakdown (groups per reason; a group can have several):" -ForegroundColor DarkGray
$reasonCounts.GetEnumerator() | Sort-Object Value -Descending | Select-Object -First 25 |
    ForEach-Object { Write-Host ("      {0,-34} {1}" -f $_.Key, $_.Value) -ForegroundColor DarkGray }

# Safety guard: a sane sensitive-group list is tens, not thousands. Above the cap,
# nested expansion = (matched x 2) recursive LDAP queries against the DC -> effectively hangs.
if ($sensitiveGroups.Count -gt $MaxGroups -and -not $Force) {
    Write-Host ""
    Write-Warn2 "STOP: $($sensitiveGroups.Count) groups matched (cap = $MaxGroups). That is almost certainly an over-broad pattern, not a real sensitive-group set."
    Write-Warn2 "The biggest offender above is likely the culprit (e.g. '*admin*'). Tighten 'groupNamePatterns' in your config.json."
    Write-Warn2 "Tips: drop catch-all wildcards like '*admin*'/'*priv*', or anchor them (e.g. 'Tier0-*', '*-DA'). Exact names like 'Domain Admins' are safest."
    Write-Warn2 "To run anyway (slow), re-run with  -Force  (optionally raise  -MaxGroups)."
    # Still write the matched-groups list so the user can review what over-matched.
    $reviewCsv = Join-Path $outFolder "sensitive-groups_OVERMATCH_$stamp.csv"
    $sensitiveGroups | Select-Object Name, sAMAccountName, MatchedPattern, DistinguishedName |
        Export-Csv $reviewCsv -NoTypeInformation -Encoding UTF8
    Write-Host "    Wrote matched list for review -> $reviewCsv" -ForegroundColor DarkGray
    return
}
...
# ----------------------------------------------------------------------------
Write-Step "Expanding nested membership (recursive) for $($sensitiveGroups.Count) group(s)..."

# Single pass over the sensitive groups, doing BOTH directions, with live progress.
#   2a. members   = who is effectively inside each group (incl. nested groups)
#   2b. memberOf  = which groups each sensitive group is nested inside
...
$sensitiveDNs    = [System.Collections.Generic.HashSet[string]]::new([string[]]$sensitiveGroups.DistinguishedName)

$total = $sensitiveGroups.Count
$sw    = [System.Diagnostics.Stopwatch]::StartNew()
$i     = 0
foreach ($grp in $sensitiveGroups) {
    $i++
    if ($i -eq 1 -or $i % 25 -eq 0 -or $i -eq $total) {
        $rate = if ($sw.Elapsed.TotalSeconds -gt 0) { [math]::Round($i / $sw.Elapsed.TotalSeconds, 1) } else { 0 }
        $eta  = if ($rate -gt 0) { [timespan]::FromSeconds([math]::Round(($total - $i) / $rate)) } else { '?' }
        Write-Host ("    nesting {0}/{1}  ({2}/s, ETA {3})" -f $i, $total, $rate, $eta) -ForegroundColor DarkGray
    }
    ...
    # 2a — recursive members via LDAP_MATCHING_RULE_IN_CHAIN (1.2.840.113556.1.4.1941)
    ...
    # 2b — recursive parents (which groups this sensitive group is nested inside)
    ...
    foreach ($par in $parents) {
        $nestingRows.Add([pscustomobject]@{
            ...
            ParentSensitive  = $sensitiveDNs.Contains($par.DistinguishedName)
        })
    }
}
$sw.Stop()
Write-Ok "Effective members: $($memberRows.Count) | Nesting links: $($nestingRows.Count) | Unique sensitive principals (incl. nested): $($sensitiveSids.Count)  [$([math]::Round($sw.Elapsed.TotalSeconds))s]"
